...
<script runat="server" enableviewstate="False">
    ...
    */
    ...
    protected void HideAndShowDetailsTabControls(OPurchaseOrder po)
    {
        ...
        //20120209 ptb
        //only enable BillTo and BillToContactPerson if not Admin
        //and if PO is a direct PO
        BillToContactPersonID.Enabled = ddlBillTo.Enabled = ddlDeliveryTo.Enabled = DeliverToContactPersonID.Enabled = cbDeliverToOther.Enabled
            = DeliverToAddress.Enabled = DeliverToPerson.Enabled = !(po.PurchaseOrderItems.Count > 0 && po.PurchaseOrderItems[0].RequestForQuotationItemID != null &&
            ddlLocation.Items[ddlLocation.SelectedIndex].Text.StartsWith("Admin"));
        cbDeliverToOther.Visible = ddlLocation.Items[ddlLocation.SelectedIndex].Text.StartsWith("Admin");
    ...
    protected void SendPOToVendor(OPurchaseOrder po)
    {
        try
        {
            ...
            if (po.IsDeliverToOther == 0)
                emailReceipients = po.ContactEmail + ";" + po.DeliverToContactPerson.UserBase.Email;
            else
                emailReceipients = po.ContactEmail;
    ...
    protected void searchUsers_Selected(object sender, EventArgs e)
    {
        ...
        {
            OUser u = TablesLogic.tUser[id];
            if (!textCCEmail.Text.Contains(u.UserBase.Email.ToString()))
                textCCEmail.Text += u.UserBase.Email.ToString() + "; ";
        }
    }
    ...
    protected void searchUsers_Searched(objectSearchDialogBox.SearchEventArgs e)
    {
        ...
        //List<OPosition> tempPositions = new List<OPosition>();
        List<OLocation> locations = new List<OLocation>();
        //List<ORole> roles = new List<ORole>();
        //roles.AddRange(ORole.GetRolesByRoleCode("APPROVER"));
        foreach (OPosition position in AppSession.User.Positions)
        {
            //if (roles.Find(lf => lf.ObjectID == position.RoleID) == null)
            //    roles.Add(position.Role);

            foreach (OLocation location in position.LocationAccess)
            {
                if (locations.Find(lf => lf.ObjectID == location.ObjectID) == null)
                {
                    locations.Add(location);
                    //List<OLocation> tempLoc = new List<OLocation>();
                    //tempLoc.Add(location);                    
                    //List<OPosition> temp = OPosition.GetPositionsAtOrBelowLocations(tempLoc, roles);
                    //    //.GetPositionsByRoleAndLocation("APPROVER", location);
                    //tempPositions.AddRange(temp);
                }
            }
        }

        //if (roles.Find(lf => lf.RoleCode == "APPROVER") == null)
        //    roles.AddRange(ORole.GetRolesByRoleCode("APPROVER"));

        //List<OUser> users = TablesLogic.tUser.LoadList(Query.True);
        //foreach (OUser user in users)
        //    user.ActivateAndSaveCurrentPositions();

        //List<OPosition> positions = OPosition.GetPositionsAtOrBelowLocations(locations, roles);
        ...
        {
            cond = cond | TablesLogic.tUser.Positions.ObjectID == p.ObjectID;
            ids.Add(p.ObjectID);
        }
        ...
        e.CustomCondition =
            (e.CustomCondition == null ? Query.True : e.CustomCondition)
            & u.isTenant == 0
            //& u.Positions.ObjectID.In(ids)
            & cond
            & u.UserBase.Email != null
            & u.UserBase.Email != ""
            ;
    }
    ...
    //    if (po != null)
    //    {
    //        return new LocationTreePopulater(po.Location.ObjectID, true, true, "OPurchaseOrder");
    //    }
    //    return new LocationTreePopulater(null, true, true, "OPurchaseOrder");
    //}
                    ...
                    <ui:UIPanel runat="server" ID="panelPOAndLOA" BorderStyle="NotSet">
                        ...
                        <ui:UIPanel runat="server" ID="POPanel" BorderStyle="NotSet" meta:resourcekey="POPanelResource1">
                            <ui:UIDialogBox runat="server" ID="dialogAdditionalAttachments" Title="Are you sure you want to proceed? Click OK to proceed, Cancel otherwise."
                                ...
                                <ui:UIPanel runat="server" ID="panelDialogAdditional" BorderStyle="NotSet">
                                    ...
                                    <ui:UIButton ID="buttonAddUsersEmails" runat="server" AlwaysEnabled="True" CausesValidation="False"
                                        ...
                                        Visible="true" OnClick="buttonAddUsersEmails_Click" Text="Add Emails" />
                                    <ui:UIFieldTextBox runat="server" ID="textCCEmail" Caption="I would like to cc. the following email(s) addresses"
                                        TextMode="MultiLine" Rows="3" CaptionPosition="Top" MaxLength="0" Hint="Please enter e-mail address in your cc list, you may want cc to mulitple e-mails by using ';' as seperator i.e.: peter@yahoo.com; mary@gmail.com">
                                    </ui:UIFieldTextBox>
                                    <web:searchdialogbox runat="server" ID="searchUsers" Title="Users" AllowMultipleSelection="true"
                                        AutoSearchOnLoad="true" BaseTable="tUser" SearchTextBoxPropertyNames="ObjectName"
                                        OnSearched="searchUsers_Searched" OnSelected="searchUsers_Selected">
                                        <Columns>
                                            <ui:UIGridViewBoundColumn HeaderText="User Name" HeaderStyle-Width="300px" PropertyName="ObjectName"
                                                SortExpression="ObjectName">
                                            </ui:UIGridViewBoundColumn>
                                            <ui:UIGridViewBoundColumn HeaderText="Email" HeaderStyle-Width="200px" PropertyName="UserBase.Email">
                                            </ui:UIGridViewBoundColumn>
                                        </Columns>
                                        <AdvancedPanel>
                                            <%--<ui:UIFieldTreeList runat="server" ID="treeLocation" Caption="Location" Width="400px"
                                                OnAcquireTreePopulater="treeLocation_AcquireTreePopulater" ShowCheckBoxes="None"
                                                TreeValueMode="SelectedNode">
                                            </ui:UIFieldTreeList>--%>
                                            <ui:UIFieldListBox runat="server" ID="listRoles" PropertyName="Positions.RoleID"
                                                Caption="Roles" meta:resourcekey="listRolesResource1" Width="400px" Rows="5">
                                            </ui:UIFieldListBox>
                                        </AdvancedPanel>
                                    </web:searchdialogbox>
                                </ui:UIPanel>
                            ...
                            <ui:UIFieldTextBox ID="DeliverToAddress" runat="server" Caption="Deliver To" TextMode="MultiLine"
                                Rows="3" PropertyName="DeliverToAddress" Visible="false" ValidateRequiredField="true">
                            </ui:UIFieldTextBox>
                            <ui:UIFieldTextBox ID="DeliverToPerson" runat="server" Caption="Deliver To Contact Person"
                                PropertyName="DeliverToPerson" Visible="false" ValidateRequiredField="true">
                            </ui:UIFieldTextBox>
