    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Session[Request["session"]] != null)
            {
                ...
                if (po.HasTaxCodePerLineItem != null && po.HasTaxCodePerLineItem == 1)
                    CrystalReportPath = "POReport3Copies_TaxPerLineItem.rpt";
                
                doc = new ReportDocument();
                doc.Load(Server.MapPath(CrystalReportPath));
                if (CrystalReportPath == "POReport3Copies.rpt" || CrystalReportPath == "POReport3Copies_TaxPerLineItem.rpt")
                {
                    DataSet ds = new DataSet();
                    DataTable dt = new DataTable();
                    dt.Columns.Add("POId");
                    dt.Rows.Add(new object[] { po.ObjectID });
                    dt.TableName = "POMainTable";
                    ds.Tables.Add(dt);
                    doc.SetDataSource(ds);
                    doc.OpenSubreport("POReport.rpt").SetDataSource(OPurchaseOrder.GetPurchaseOrderDataSetForCrystalReports(po));
                    doc.OpenSubreport("POReport.rpt - 01").SetDataSource(OPurchaseOrder.GetPurchaseOrderDataSetForCrystalReports(po));
                    doc.OpenSubreport("POReport.rpt - 02").SetDataSource(OPurchaseOrder.GetPurchaseOrderDataSetForCrystalReports(po));
                ...
                Response.AddHeader("content-disposition", "attachment; filename=" + po.ObjectNumber + ".pdf");
