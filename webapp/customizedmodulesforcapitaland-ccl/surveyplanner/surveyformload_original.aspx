...
<script runat="server">
    ...
    protected void form1_OnLoad(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (Request["SPID"] != null && Request["SRPID"] != null && Request["REA"] != null)
            {
                Guid SPID = Security.DecryptGuid(Request["SPID"]);
                OSurveyPlanner SP = TablesLogic.tSurveyPlanner.Load(SPID);
                Guid SRPID = Security.DecryptGuid(Request["SRPID"]);//201109
                ...
                ArrayList AL_SurveyID = Query.SelectDistinct(TablesLogic.tSurvey.ObjectID)
                    ...
                    TablesLogic.tSurvey.SurveyResponseFroms.SurveyRespondent.SurveyRespondentPortfolioID == SRPID &//201109
                ...
                if (AL_SurveyID != null && AL_SurveyID.Count > 0)
                    LoadSurveyForm((Guid)AL_SurveyID[0], SRPID, RespondentEmailAddress);
    ...
    {
        Guid SurveyRespondentPortfolioID = Security.DecryptGuid(Request["SRPID"]);
        ...
        List<OSurveyChecklistItem> ListOfSCLI = TablesLogic.tSurveyChecklistItem.LoadList(
           ...
           TablesLogic.tSurveyChecklistItem.SurveyRespondentPortfolioID == SurveyRespondentPortfolioID &
           TablesLogic.tSurveyChecklistItem.SurveyRespondentPortfolio.SurveyRespondents.EmailAddress.UpperCase() == RespondentEmailAddress.ToUpper()
           ,
    ...
    {
        Guid SRPID = Security.DecryptGuid(Request["SRPID"]);
        ...
        foreach (object O in AL_SurveyID)
        {
            List<OSurveyChecklistItem> ListOfSCLIByOthers = TablesLogic.tSurveyChecklistItem.LoadList(
                ...
                TablesLogic.tSurveyChecklistItem.SurveyRespondentPortfolioID == SRPID &
    ...
    public void LoadSurveyForm(Guid SurveyID, Guid SurveyRespondentPortfolioID, string RespondentEmailAddress)
    {
        bool IsModifiable = true;
        List<OSurveyChecklistItem> ListOfSCLIByOthers = TablesLogic.tSurveyChecklistItem.LoadList(
            TablesLogic.tSurveyChecklistItem.SurveyID == SurveyID &
            TablesLogic.tSurveyChecklistItem.SurveyRespondentPortfolioID == SurveyRespondentPortfolioID &//201109
            TablesLogic.tSurveyChecklistItem.Status == SurveyStatusType.ValidWithReply
            );
    ...
    protected void btn_SaveSurvey_Click(object sender, EventArgs e)
    {
        ...
        LoadSurveyForm((Guid)Session["CurrentSurveyID"], Security.DecryptGuid(Request["SRPID"]), Security.Decrypt(Request["REA"]));
    ...
    protected void btn_NavigatePrevious_Click(object sender, EventArgs e)
    {
        ...
        int CurrentIndex = AL_SurveyID.IndexOf(CurrentSurveyID);
        if (CurrentIndex != 0)
            LoadSurveyForm((Guid)AL_SurveyID[CurrentIndex - 1], Security.DecryptGuid(Request["SRPID"]), Security.Decrypt(Request["REA"]));
    }
