...
<script runat="server">
    ...
    protected void panel_ValidateAndSave(object sender, EventArgs e)
    {
        using (Connection c = new Connection())
        {
            ...
            //Validate the WorkCost
            if (storeCheckIn.WorkID != null)
            {
                foreach (OStoreCheckInItem storeCheckInItem in storeCheckIn.CheckInItems)
                {
                    if (storeCheckInItem.FromWorkCost.ActualQuantity != 0)
                    {
                        if (storeCheckInItem.Quantity > storeCheckInItem.FromWorkCost.ActualQuantity)
                            gridCheckInItem.ErrorMessage = Resources.Errors.StoreCheckIn_InvalidReturnStock;
                    }
                    else if (objectBase.SelectedAction.Is("Commit", "SubmitForApproval", "Approve"))
                        gridCheckInItem.ErrorMessage = Resources.Errors.StoreCheckIn_RequiredCheckOutFirst;
                }
            }
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        */
        ...
        panelDestinationWork.Enabled = StoreID.SelectedValue != "";
        buttonAddEquipment.Visible = buttonAddItems.Visible = gridCheckInItem.Commands[1].Visible = !(WorkID.ControlValue != null);

        CheckIn_UnitPrice.Enabled = checkin.WorkID == null;
    ...
    protected void CheckInItem_SubPanel_ValidateAndUpdate(object sender, EventArgs e)
    {
        ...
        if (storeCheckInItem.FromWorkCostID != null)
        {
            if (storeCheckInItem.FromWorkCost.ActualQuantity != null)
            {
                if (storeCheckInItem.Quantity > storeCheckInItem.FromWorkCost.ActualQuantity)
                    CheckIn_Quantity.ErrorMessage = Resources.Errors.StoreCheckIn_InvalidReturnStock;
            }
            else
                CheckIn_Quantity.ErrorMessage = Resources.Errors.StoreCheckIn_RequiredCheckOutFirst;
        }

        if (!CheckInItem_SubPanel.ObjectPanel.IsValid)
            return;
    ...
    protected void searchWork_Selected(object sender, EventArgs e)
    {
        ...
        storeCheckIn.WorkID = id;

        storeCheckIn.CheckInItems.Clear();

        OWork work = storeCheckIn.Work;
        foreach (OWorkCost wc in work.WorkCost)
        {
            if (wc.CostType == (int)WorkCostType.Material && wc.StoreID == storeCheckIn.StoreID)
            {
                OStoreCheckInItem newScii = TablesLogic.tStoreCheckInItem.Create();
                newScii.FromWorkCostID = wc.ObjectID;
                newScii.StoreBinID = wc.StoreBinID;
                newScii.CatalogueID = wc.CatalogueID;
                newScii.Quantity = wc.ActualQuantity;
                newScii.UnitPrice = wc.ActualUnitCost;                

                storeCheckIn.CheckInItems.Add(newScii);
            }
        }
    ...
    <ui:UIObjectPanel runat="server" ID="panelMain" BorderStyle="NotSet" meta:resourcekey="panelMainResource1">
        ...
        <div class="div-main">
            <ui:UITabStrip runat="server" ID="tabObject" meta:resourcekey="tabObjectResource1"
                BorderStyle="NotSet">
                <ui:UITabView runat="server" ID="tabDetails" Caption="Check In" meta:resourcekey="uitabview1Resource1"
                    BorderStyle="NotSet">
                    <web:base runat="server" ID="objectBase" ObjectNumberVisible="false" ObjectNameVisible="false"
                        ObjectNumberEnabled="false" ObjectNumberCaption="Check-In Number" meta:resourcekey="objectBaseResource1">
                    </web:base>
                    <ui:UIPanel runat="server" ID="panelDetails" BorderStyle="NotSet" meta:resourcekey="panelDetailsResource1">
                        ...
                        <ui:UIPanel runat="server" ID="panelDestinationWork" meta:resourcekey="panelDestinationWorkResource1"
                            ...
                            <ui:UIFieldDialogSelection runat="server" ID="WorkID" PropertyName="WorkID" PropertyNameItem="Work.ObjectNumber"
                                Span="Half" Caption="Work Order" ImageClearUrl="~/images/cross.gif" ValidateRequiredField="false"
                        ...
                        <ui:UIFieldSearchableDropDownList runat="server" ID="StoreCheckOutID" PropertyName="StoreCheckOutID"
                            Caption="Store Check Out" meta:resourcekey="StoreCheckOutIDResource1" SearchInterval="300">
                            <ContextMenuButtons>
                                <ui:UIButton ID="buttonViewStoreCheckout" runat="server" AlwaysEnabled="True" CausesValidation="False"
                                    ConfirmText="Please remember to save this Store Check In\n\nAre you sure you want to continue?"
                                    ImageUrl="~/images/view.gif" meta:resourcekey="uttonViewStoreCheckoutResource1"
                                    OnClick="buttonViewStoreCheckout_Click" Text="View Store Checkout" />
                            </ContextMenuButtons>
                        </ui:UIFieldSearchableDropDownList>
                        <ui:UIFieldTextBox TextMode="MultiLine" runat="server" ID="Description" PropertyName="Description"
                            Caption="Remarks" meta:resourcekey="DescriptionResource1" InternalControlWidth="95%">
                        </ui:UIFieldTextBox>
                        <br />
                        <ui:UISeparator runat="server" ID="sep1" meta:resourcekey="sep1Resource1"></ui:UISeparator>
                        <ui:UIButton runat="server" ID="buttonAddItems" OnClick="buttonAddItems_Click" Text="Add Multiple Items"
                            ImageUrl="~/images/add.gif" CausesValidation="False" />
                        <ui:UIButton runat="server" ID="buttonAddEquipment" OnClick="buttonAddEquipment_Click"
                            Text="Add Equipment" ImageUrl="~/images/add.gif" CausesValidation="False" meta:resourcekey="buttonAddEquipmentResource1" />
                        <ui:UIButton runat="server" ID="buttonItemsAdded" OnClick="buttonItemsAdded_Click"
                            CausesValidation="False" meta:resourcekey="buttonItemsAddedResource1" />
                        <br />
                        <br />
                        <ui:UIGridView runat="server" ID="gridCheckInItem" PropertyName="CheckInItems" Caption="Check In Items"
                            KeyName="ObjectID" meta:resourcekey="gridCheckInItemResource1" Width="100%" ValidateRequiredField="True"
                            PageSize="1000" ShowFooter='True' DataKeyNames="ObjectID" GridLines="Both" RowErrorColor=""
                            ...
                            <Columns>
                                ...
                                <cc1:UIGridViewBoundColumn DataField="Catalogue.InventoryCatalogTypeText" HeaderText="Type"
                                    meta:resourcekey="UIGridViewBoundColumnResource1" PropertyName="Catalogue.InventoryCatalogTypeText"
                                    ResourceAssemblyName="" SortExpression="Catalogue.InventoryCatalogTypeText">
                                    <HeaderStyle HorizontalAlign="Left" />
                                    <ItemStyle HorizontalAlign="Left" />
                                </cc1:UIGridViewBoundColumn>
                                <cc1:UIGridViewBoundColumn DataField="Catalogue.ObjectName" HeaderText="Check in item"
                                ...
                                <cc1:UIGridViewBoundColumn DataField="Catalogue.StockCode" HeaderText="Stock Code"
                                    meta:resourcekey="UIGridViewBoundColumnResource2" PropertyName="Catalogue.StockCode"
                                ...
                                <cc1:UIGridViewBoundColumn DataField="UnitPrice" DataFormatString="{0:#,##0.00}"
                                    HeaderText="Unit Price" meta:resourceKey="UIGridViewColumnResource5" PropertyName="UnitPrice"
                                    ResourceAssemblyName="" SortExpression="UnitPrice">
                                    <HeaderStyle HorizontalAlign="Left" />
                                    <ItemStyle HorizontalAlign="Left" />
                                </cc1:UIGridViewBoundColumn>
                                <cc1:UIGridViewBoundColumn DataField="Quantity" HeaderText="Quantity" meta:resourceKey="UIGridViewColumnResource6"
                                    PropertyName="Quantity" ResourceAssemblyName="" SortExpression="Quantity" DataFormatString="{0:#,##0.00}">
                                    <HeaderStyle HorizontalAlign="Left" />
                                    <ItemStyle HorizontalAlign="Left" />
                                </cc1:UIGridViewBoundColumn>
                                ...
                                <cc1:UIGridViewBoundColumn DataField="SubTotal" DataFormatString="{0:#,##0.00}" FooterAggregate="Sum"
                                    HeaderText="SubTotal" meta:resourcekey="UIGridViewBoundColumnResource4" PropertyName="SubTotal"
                        ...
                        <ui:UIObjectPanel runat="server" ID="CheckInItem_Panel" meta:resourcekey="CheckInItem_PanelResource1"
                            BorderStyle="NotSet">
                            <web:subpanel runat="server" ID="CheckInItem_SubPanel" GridViewID="gridCheckInItem"
                                OnPopulateForm="CheckInItem_SubPanel_PopulateForm" MultiSelectGridID="CheckIn_MultiCatalogueID"
                                MultiSelectColumnNames="CatalogueID,StoreBinID,UnitPrice,Quantity,ExpiryDate,LotNumber"
                                OnValidateAndUpdate="CheckInItem_SubPanel_ValidateAndUpdate" />
                            <ui:UIPanel runat="server" ID="panelCatalogDetail" BorderStyle="NotSet" meta:resourcekey="panelCatalogDetailResource1">
                                ...
                                <ui:UIFieldTreeList runat="server" ID="CheckIn_CatalogueID" PropertyName="CatalogueID"
                                    ...
                                    ToolTip="Catalog item to check in" meta:resourcekey="CheckIn_CatalogueIDResource1"
                                ...
                                <ui:UIFieldTextBox runat="server" ID="CheckIn_UnitPrice" PropertyName="UnitPrice"
                                    DataFormatString="{0:#,##0.00}" Span="Half" ValidateRequiredField="True" ValidationDataType="Currency"
                                    Caption="Unit Price" ValidateRangeField="True" ValidationRangeMin="0" ValidationRangeType="Currency"
                                ...
                                <ui:UIFieldTextBox runat="server" ID="CheckIn_Quantity" PropertyName="Quantity" Span="Half"
                                    DataFormatString="{0:#,##0.00}" ValidateRangeField="True" ValidationRangeType='Currency'
                                    ValidationRangeMin="0" ValidationRangeMinInclusive="False" ValidateRequiredField="True"
                                    ValidationDataType="Currency" Caption="Quantity" ToolTip="Quantity to check-in."
                                ...
                                <ui:UIPanel runat="server" ID="panelInventory" BorderStyle="NotSet" meta:resourcekey="panelInventoryResource1">
