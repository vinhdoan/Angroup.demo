...
<script runat="server">
    ...
    private Hashtable EditWorkVisible = new Hashtable();
    private Hashtable ViewWorkVisible = new Hashtable();
    ...
    protected void panel_PopulateForm(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (requestForQuotation.TransactionTypeGroupID != null)
                dropPurchaseType.Bind(OCode.GetCodesByTypeAndParentID("PurchaseType", requestForQuotation.TransactionTypeGroupID, requestForQuotation.PurchaseTypeID));
            else
        ...
        bool isfromRFQ = false;
        if (requestForQuotation.RequestForQuotationItems.Count > 0 && requestForQuotation.RequestForQuotationItems[0].WorkCostID != null)
            isfromRFQ = true;
        rdlRecoverableType.Bind(ORequestForQuotation.ListRecoverableType(isfromRFQ), "Text", "Value");
    ...
    protected void RequestForQuotationItem_SubPanel_PopulateForm(object sender, EventArgs e)
    {
        ...
        ChargeAmount.Visible = rdlRecoverableType.SelectedValue == "1";
        RecoverableAmount.Visible = rdlRecoverableType.SelectedValue == "1" || rdlRecoverableType.SelectedValue == "2";
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        searchMultipleVendor.Enabled = !RequestForQuotationVendors_SubPanel.Visible;
        ...
        // regardless it's term contract or not.
        //
        buttonGeneratePOFromLineItems.Visible = radioBudgetDistributionMode.SelectedValue == "1";
        buttonGenerateLOAFromLineItems.Visible = radioBudgetDistributionMode.SelectedValue == "1";
        buttonGeneratePO.Visible = radioBudgetDistributionMode.SelectedValue == "0";
        buttonGenerateLOA.Visible = radioBudgetDistributionMode.SelectedValue == "0";
        ...
        {
            if (rfqVendor.IsExchangeRateDefined == 1)
        ...
        if (rfq.IsNew)
        {
            if (rfq.TransactionTypeGroupID == null && dropPurchaseTypeClassification.Items.Count == 1)
            {
                ...
                dropPurchaseType.Bind(OCode.GetCodesByTypeAndParentID("PurchaseType", new Guid(dropPurchaseTypeClassification.SelectedValue), rfq.PurchaseTypeID));
            }
        ...
        //RequestorName.ValidateRequiredField =  rdlIsRecoverable.SelectedValue == "1";
        //tabBudget.Visible = rdlIsRecoverable.SelectedValue != "1";
        rdlRecoverableType.Enabled = rfq.RequestForQuotationItems.Count == 0;
    }
    /// <summary>
    ...
    protected void Workflow_Setting()
    {
        ...
        //
        if (objectBase.CurrentObjectState.Is("PendingApproval"))
        {
            ListItem item = objectBase.GetWorkflowRadioListItem("Cancel");
            if (item != null)
                item.Enabled = false;
        }
    ...
    protected void popupGeneratePO_ButtonClicked(object sender, ButtonClickedEventArgs e)
    {
        if (e.CommandName == "Confirm")
        {
            ...
            foreach (GridViewRow row in gridGeneratePO.Rows)
            {
                ...
                if (orderQuantity <= 0)
                {
                    tb.ErrorMessage = "Order Quantity cannot be zero or below zero.";
                    return;
                }
    ...
    protected void gridBudgetSummary_Action(object sender, string commandName, List<object> dataKeys)
    {
        if (commandName == "ViewBudget")
        {
            ...
            if (budgetSummary != null)
                Window.Open("../../customizedmodulesforcapitaland-my/budgetperiod/budgetview.aspx?ID=" +
    ...
    protected void gridAwardItems_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (rdlRecoverableType.SelectedValue == "1" || rdlRecoverableType.SelectedValue == "2")
                e.Row.Cells[13].Visible = true;
            else
                e.Row.Cells[13].Visible = false;
            #region
            // 2011.06.23, Kien Trung
            // Commented out, do hide/show command button at RowDataBound gridPurchaseOrderItems
            // as now multiple POs to one PR.
            //
    ...
    protected void RequestForQuotationItems_Action(object sender, string commandName, List<object> objectIds)
    {
        ...
        else if (commandName == "EditWork")
        {
            Guid? objectID = (Guid?)objectIds[0];
            ORequestForQuotationItem item = TablesLogic.tRequestForQuotationItem[objectID.Value];
            Window.OpenEditObjectPage(this, "OWork", item.WorkCost.WorkID.ToString(), "");
        }
        else if (commandName == "ViewWork")
        {
            Guid? objectID = (Guid?)objectIds[0];
            ORequestForQuotationItem item = TablesLogic.tRequestForQuotationItem[objectID.Value];
            Window.OpenViewObjectPage(this, "OWork", item.WorkCost.WorkID.ToString(), "");
        }
    }
    ...
    protected void RequestForQuotationItems_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            Guid rfqItemID = (Guid)RequestForQuotationItems.DataKeys[e.Row.RowIndex][0];
            ORequestForQuotationItem item = TablesLogic.tRequestForQuotationItem[rfqItemID];
            if (item != null && item.WorkCost != null)
            {
                Guid workID = item.WorkCost.WorkID.Value;
                if (EditWorkVisible[workID] == null)
                    EditWorkVisible.Add(workID, OWork.IsObjectEditOrView(AppSession.User, workID, "OWork", true));
                if (ViewWorkVisible[workID] == null)
                    ViewWorkVisible.Add(workID, OWork.IsObjectEditOrView(AppSession.User, workID, "OWork", false));

                e.Row.Cells[10].Visible = Convert.ToBoolean(EditWorkVisible[workID]);
                e.Row.Cells[11].Visible = Convert.ToBoolean(ViewWorkVisible[workID]);
            }
            else
            {
                e.Row.Cells[10].Visible = false;
    ...
    protected void gridGeneratePO_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (e.Row.Cells[4].Text != "" && e.Row.Cells[4].Text != "&nbsp;")
            {
                ...
                // Disable the text box for marcom and ops.
                //
                if (tb != null && ConfigurationManager.AppSettings["CustomizedInstance"] != "IT")
                {
                    tb.Enabled = false;
                }
            }
            ...
            if (e.Row.Cells[8].Text != "&nbsp;")
                e.Row.Cells[8].Text = Convert.ToDecimal(e.Row.Cells[8].Text).ToString(OApplicationSetting.Current.BaseCurrency.CurrencySymbol + "#,##0.00##");
    ...
    protected void searchTenantLease_Selected(object sender, EventArgs e)
    {
        ...
        obj.RequestorID = tenantLease.TenantID;
    ...
    protected void RequestForQuotationItems_RowCommand(object sender, GridViewCommandEventArgs e)
    {
...
<body>
    ...
    <ui:UIObjectPanel runat="server" BorderStyle="NotSet" meta:resourcekey="UIObjectPanelResource1">    
        <web:object runat="server" ID="panel" Caption="Work Justification" BaseTable="tRequestForQuotation"
            ...
            SaveButtonsVisible="false" ShowWorkflowActionAsButtons="true"
        ...
        <div class="div-main">
            <ui:UITabStrip runat="server" ID="tabObject" meta:resourcekey="tabObjectResource1"
                ...
                <ui:UITabView ID="tabDetails" runat="server" Caption="Details" meta:resourcekey="tabDetailsResource1"
                    ...
                    <ui:UIPanel runat="server" ID="panelDetails" meta:resourcekey="panelDetailsResource1"
                        ...
                        <ui:UIPanel runat="server" ID="panelDetails2">
                            ...
                            <ui:UIPanel runat="server" ID="Panel_Requestor">
                            <ui:UIFieldRadioList runat="server" ID="rdlRecoverableType" PropertyName="RecoverableType" Caption="Recoverable Type" OnSelectedIndexChanged="rdlRecoverableType_SelectedIndexChanged">
                                <%--<Items>
                                    <asp:ListItem Value="0" Text="Non-Recoverable"></asp:ListItem>
                                    <asp:ListItem Value="1" Text="Recoverable from Tenant"></asp:ListItem>
                                    <asp:ListItem Value="2" Text="Recoverable from Third Party"></asp:ListItem>
                                </Items>--%>
                            </ui:UIFieldRadioList>
                                <ui:UISeparator runat="server" ID="RequestorSeparator" Caption="Requestor" />
                                <%--<ui:UIFieldSearchableDropDownList runat="server" ID="RequestorID" PropertyName="RequestorID" Caption="Requestor" ToolTip="The Requestor that made the request for this work justification. Leave this empty if the Requestor does not exist in this list. You can key in his or her name below in the 'Name' field." MaximumNumberOfItems="100" SearchInterval="300">
                                </ui:UIFieldSearchableDropDownList>--%>
                                <ui:UIFieldLabel runat="server" ID="RequestorID" PropertyName="Requestor.ObjectName" Caption="Tenant" Span="Half" ToolTip="The Requestor that made the request for this work justification. Leave this empty if the Requestor does not exist in this list. You can key in his or her name below in the 'Name' field."></ui:UIFieldLabel>
                                <web:searchdialogbox runat='server' ID="searchTenantLease" BaseTable="tTenantLease" AutoSearchOnLoad="false" AllowMultipleSelection="false"
                                    ButtonSelectText="Select tenant from leases" Title="Select A Tenant Lease" MaximumNumberOfResults="100"
                                    SearchTextBoxPropertyNames="Tenant.ObjectName, Location.ObjectName, ShopName, Location.Parent.ObjectName" OnSearched="searchTenantLease_Searched" OnSelected="searchTenantLease_Selected" OnCleared="searchTenantLease_Cleared">
                                    <Columns>
                                        <ui:UIGridViewButtonColumn AlwaysEnabled="true" CommandName="AddObject" ImageUrl="~/images/tick.gif"></ui:UIGridViewButtonColumn>
                                        <ui:UIGridViewBoundColumn HeaderText="Tenant Name" HeaderStyle-Width="300px" PropertyName="Tenant.ObjectName"></ui:UIGridViewBoundColumn>
                                        <ui:UIGridViewBoundColumn HeaderText="Unit No" HeaderStyle-Width="100px" PropertyName="Location.ObjectName"></ui:UIGridViewBoundColumn>
                                        <ui:UIGridViewBoundColumn HeaderText="Shop Name" HeaderStyle-Width="100px" PropertyName="ShopName"></ui:UIGridViewBoundColumn>
                                ...
                                <ui:UIFieldTextBox runat="server" ID="RequestorName" Caption="Name" PropertyName="RequestorName" ToolTip="The name of the Requestor to refer by." InternalControlWidth="95%"></ui:UIFieldTextBox>
                                <ui:UIFieldTextBox ID="RequestorCellPhone" runat="server" Caption="Cell Phone" PropertyName="RequestorCellPhone" Span="Half"  InternalControlWidth="95%" />
                                <ui:UIFieldTextBox ID="RequestorEmail" runat="server" Caption="Email" PropertyName="RequestorEmail" Span="Half"  InternalControlWidth="95%" />
                                <ui:UIFieldTextBox ID="RequestorFax" runat="server" Caption="Fax" PropertyName="RequestorFax" Span="Half"  InternalControlWidth="95%" />
                                <ui:UIFieldTextBox ID="RequestorPhone" runat="server" Caption="Phone" PropertyName="RequestorPhone" Span="Half"  InternalControlWidth="95%" />
                            </ui:UIPanel>
                            <ui:UISeparator runat="server" ID="sepOtherDetails" Caption="Other Information" />
                ...
                <ui:UITabView runat="server" ID="tabLineItems" Caption="Line Items" meta:resourcekey="tabLineItemsResource1"
                    ...
                    <ui:UIPanel runat="server" ID="panelRequestForQuotationItems" meta:resourcekey="panelRequestForQuotationItemsResource1"
                        ...
                        <ui:UIGridView ID="RequestForQuotationItems" runat="server" Caption="Items" PropertyName="RequestForQuotationItems"
                            ...
                            OnRowDataBound="RequestForQuotationItems_RowDataBound" OnRowCommand="RequestForQuotationItems_RowCommand">
                            ...
                            <Columns>
                                ...
                                <cc1:UIGridViewBoundColumn DataField="RecoverableAmount" DataFormatString="{0:n}"
                                    HeaderText="Recoverable Amount" PropertyName="RecoverableAmount"
                                    ResourceAssemblyName="" SortExpression="RecoverableAmount">
                                    <HeaderStyle HorizontalAlign="Left" />
                                    <ItemStyle HorizontalAlign="Left" />
                                </cc1:UIGridViewBoundColumn>
                                <ui:UIGridViewButtonColumn ButtonType="Image" CommandName="EditWork" ImageUrl="~/images/edit.gif" AlwaysEnabled="true" 
                                        ConfirmText="Are you sure you wish to open this Work for editing? Please remember to save the Work Justification, otherwise changes that you have made will be lost.">
                                    <HeaderStyle HorizontalAlign="Left" Width="16px" />
                                    <ItemStyle HorizontalAlign="Left" />
                                </ui:UIGridViewButtonColumn>
                                <ui:UIGridViewButtonColumn ButtonType="Image" CommandName="ViewWork" ImageUrl="~/images/view.gif" AlwaysEnabled="true"
                                     ConfirmText="Are you sure you wish to open this Work for viewing? Please remember to save the Work Justification, otherwise changes that you have made will be lost.">
                                    <HeaderStyle HorizontalAlign="Left" Width="16px" />
                                    <ItemStyle HorizontalAlign="Left" />
                                </ui:UIGridViewButtonColumn>
                                <ui:UIGridViewBoundColumn DataField="CopiedFromWorkObjectNumber" HeaderText="Copied From"
                                    PropertyName="CopiedFromWorkObjectNumber"
                                    ResourceAssemblyName="" SortExpression="CopiedFromWorkObjectNumber">
                                    <HeaderStyle HorizontalAlign="Left" />
                                    <ItemStyle HorizontalAlign="Left" />
                                </ui:UIGridViewBoundColumn>
                            </Columns>
                        ...
                        <ui:UIObjectPanel ID="RequestForQuotationItem_Panel" runat="server" meta:resourcekey="RequestForQuotationItem_PanelResource1"
                            ...
                            <ui:UIFieldTextBox runat="server" ID="RecoverableAmount" Caption="Recoverable Amount" PropertyName="RecoverableAmount" Span="Half" ValidateRequiredField="true"></ui:UIFieldTextBox>
                ...
                <ui:UITabView runat="server" ID="tabQuotations" Caption="Vendors & Quotations" meta:resourcekey="tabQuotationsResource1"
                    ...
                    <br />
                    <br />
                    <web:searchdialogbox runat="server" ID="searchMultipleVendor" AllowMultipleSelection="true" BaseTable="tVendor"
                        Title="Add Multiple Vendor" AutoSearchOnLoad="false" ButtonSelectText="Add Multiple Vendors"
                        MaximumNumberOfResults="200" SearchTextBoxPropertyNames="ObjectName,OperatingContactPerson"
                        OnSelected="searchMultipleVendor_Selected" OnSearched="searchMultipleVendor_Searched">
                        <Columns>
                            <ui:UIGridViewBoundColumn HeaderText="Vendor Name" PropertyName="ObjectName" DataField="ObjectName" HeaderStyle-Width="200px"></ui:UIGridViewBoundColumn>
                            <ui:UIGridViewBoundColumn HeaderText="Operating Address" PropertyName="OperatingAddress" DataField="OperatingAddress" HeaderStyle-Width="300px"></ui:UIGridViewBoundColumn>
                            <ui:UIGridViewBoundColumn HeaderText="Contact Person" PropertyName="OperatingContactPerson" DataField="OperatingContactPerson" HeaderStyle-Width="150px"></ui:UIGridViewBoundColumn>
                        </Columns>
                    </web:searchdialogbox>
                    ...
                    <ui:UIObjectPanel ID="RequestForQuotationVendors_Panel" runat="server" meta:resourcekey="RequestForQuotationVendors_PanelResource1"
                        BorderStyle="NotSet">
                        <web:subpanel runat="server" ID="RequestForQuotationVendors_SubPanel" GridViewID="RequestForQuotationVendors" UpdatePopupVisible="false"
                            ...
                            <ui:UIGridView ID="RequestForQuotationVendorItems" runat="server" Caption="Quotation"
                                ...
                                <Columns>
                                    ...
                                    <cc1:UIGridViewTemplateColumn HeaderText="Quantity Provided" meta:resourceKey="UIGridViewColumnResource23">
                                        <ItemTemplate>
                                            <cc1:UIFieldTextBox ID="QuantityProvided" runat="server" Caption="Quantity" CaptionWidth="1px" ShowCaption="false"
                                    ...
                                    <cc1:UIGridViewTemplateColumn HeaderText="Unit Price" meta:resourceKey="UIGridViewColumnResource24">
                                        <ItemTemplate>
                                            <cc1:UIFieldTextBox ID="UnitPrice" runat="server" Caption="Unit Price" CaptionWidth="1px" ShowCaption="false"
                ...
                <ui:UITabView runat="server" ID="tabAward" Caption="Award" meta:resourcekey="tabAwardResource1"
                    ...
                    <ui:UIPanel runat='server' ID="panelQuotations">
                        <ui:UIGridView ID="gridAwardItems" runat="server" Caption="Items" PropertyName="RequestForQuotationItems"
                            SortExpression="ItemNumber" KeyName="ObjectID" ShowFooter="True" PageSize="200" BindObjectsToRows="true"
                            ...
                            <Columns>
                                ...
                                <cc1:UIGridViewBoundColumn DataField="QuantityOrdered" DataFormatString="{0:#,##0.00##}"
                                    ...
                                    PropertyName="QuantityOrdered" ResourceAssemblyName="" SortExpression="QuantityOrdered">
                                    <HeaderStyle HorizontalAlign="Left" />
                                    <ItemStyle HorizontalAlign="Left" />
                                </cc1:UIGridViewBoundColumn>
                                ...
                                <cc1:UIGridViewBoundColumn DataField="RecoverableAmount" DataFormatString="{0:c}" FooterAggregate="Sum"
                                    HeaderText="Recoverable Amount" HtmlEncode="False"
                                ...
                                <ui:UIGridViewTemplateColumn HeaderText="Copied To">
                                    ...
                                    <ItemTemplate>
                                        <ui:UIGridView runat='server' ID="gridPurchaseOrderItems"
                                            PropertyName="PurchaseOrderItems" SortExpression="PurchaseOrder.ObjectNumber ASC"
                                            AllowPaging="false" ShowCaption="false" FieldLayout="Flow" EnableTheming="false"
                                            DataKeyNames="ObjectID" CheckBoxColumnVisible="false"
                                            ShowHeader="false" OnRowCommand="gridPurchaseOrderItems_RowCommand" OnRowDataBound="gridPurchaseOrderItems_RowDataBound">
                                        <Columns>
                                            <cc1:UIGridViewBoundColumn DataField="PurchaseOrderID" HeaderText=""
                                                meta:resourcekey="UIGridViewBoundColumnResource20" PropertyName="PurchaseOrderID"
                                                ResourceAssemblyName="" SortExpression="PurchaseOrderID">
                                                <HeaderStyle HorizontalAlign="Left" />
                                                <ItemStyle HorizontalAlign="Left" />
                                            </cc1:UIGridViewBoundColumn>
                                            <ui:UIGridViewButtonColumn ButtonType="Image" CommandName="EditPO" ImageUrl="~/images/edit.gif" AlwaysEnabled="true" 
                                            ...
                                            <cc1:UIGridViewBoundColumn DataField="PurchaseOrder.ObjectNumber" HeaderText=""
                                                meta:resourcekey="UIGridViewBoundColumnResource20" PropertyName="PurchaseOrder.ObjectNumber"
                        ...
                        <ui:UIPanel runat="server" ID="panelGeneratePOButton" meta:resourcekey="panelGeneratePOButtonResource1"
                            ...
                            <ui:UIButton runat="server" ID="buttonGeneratePOFromLineItems" Text="Generate PO from Selected Line Items"
                                ImageUrl="~/images/add.gif" OnClick="buttonGeneratePOFromLineItems_Click"
                            ...
                            <ui:UIButton runat="server" ID="buttonGeneratePO" Text="Generate PO for all items"
                                ImageUrl="~/images/add.gif" OnClick="buttonGeneratePO_Click"
                    ...
                    <ui:UIDialogBox runat="server" ID="popupGeneratePO" Title="Are you sure you wish to generate Purchase Order / LOA for these Purchase Requisition items?" DialogWidth="700px"
                        Button1CausesValidation="true" Button1AutoClosesDialogBox="true" Button1Text="OK" Button1AlwaysEnabled="true" 
                        Button1CommandName="Confirm" Button1FontBold="true" Button1ImageUrl="~/images/add.gif"
                        Button2CausesValidation="false" Button2AutoClosesDialogBox="true" Button2Text="Cancel" Button2AlwaysEnabled="true" 
                        Button2CommandName="Cancel" Button2FontBold="true" Button2ImageUrl="~/images/delete.gif"
                        OnButtonClicked="popupGeneratePO_ButtonClicked">
                        <ui:UIGridView ID="gridGeneratePO" runat="server" Caption="Items" 
                            KeyName="ObjectID" ShowFooter="True" PageSize="200" CheckBoxColumnVisible="false"
                            Width="100%" PagingEnabled="True" OnRowDataBound="gridGeneratePO_RowDataBound"
                            DataKeyNames="ObjectID" GridLines="Both" SortExpression="ItemNumber ASC"
                            RowErrorColor="" Style="clear: both;" ImageRowErrorUrl="">
                        <PagerSettings Mode="NumericFirstLast" Visible="False" />
                        <Columns>
                            <cc1:UIGridViewBoundColumn DataField="ItemNumber" HeaderText="Number" 
                                PropertyName="ItemNumber" ResourceAssemblyName="" SortExpression="ItemNumber">
                                <HeaderStyle HorizontalAlign="Left" />
                                <ItemStyle HorizontalAlign="Left" />
                            </cc1:UIGridViewBoundColumn>
                            <cc1:UIGridViewBoundColumn DataField="ItemDescription" HeaderText="Description" 
                                PropertyName="ItemDescription" ResourceAssemblyName="" SortExpression="ItemDescription">
                                <HeaderStyle HorizontalAlign="Left" />
                                <ItemStyle HorizontalAlign="Left" />
                            </cc1:UIGridViewBoundColumn>
                            ...
                            <ui:UIGridViewTemplateColumn HeaderText="Order Quantity">
                                <HeaderStyle Font-Bold="true" />
                                <ItemTemplate>
                                    <ui:UIFieldTextBox ID="generateQty" runat="server" ShowCaption="false" CaptionWidth="1px"
                                        FieldLayout="Flow" InternalControlWidth="60px" 
                                        PropertyName="" ValidateDataTypeCheck="True" ValidateRangeField="True"
                                        ValidateRequiredField="True" ValidationDataType="Currency" ValidationRangeMax="99999999999999"
                                        ValidationRangeMin="0" ValidationRangeType="Currency">
                                    </ui:UIFieldTextBox>
                                </ItemTemplate>
                ...
                <ui:UITabView runat="server" ID="tabBudget" Caption="Budget" BorderStyle="NotSet"
                    ...
                    <ui:UIPanel runat="server" ID="panelBudget2">
                        ...
                        <ui:UIGridView runat="server" ID="gridBudget" PropertyName="PurchaseBudgets" Caption="Budgets"
                            ...
                            <Columns>
                                <cc1:UIGridViewButtonColumn ButtonType="Image" CommandName="EditObject" ImageUrl="~/images/edit.gif" AlwaysEnabled="true"
                        ...
                        <ui:UIObjectPanel runat="server" ID="panelBudget" BorderStyle="NotSet" meta:resourcekey="panelBudgetResource1">
                            <web:subpanel runat="server" ID="subpanelBudget" GridViewID="gridBudget" UpdatePopupVisible="true"
                                OnPopulateForm="subpanelBudget_PopulateForm" OnValidateAndUpdate="subpanelBudget_ValidateAndUpdate" OnRemoved="subpanelBudget_Removed" />
                                <ui:UIFieldDropDownList runat="server" ID="dropAddBudgetItemNumber" Caption="Line Number" PropertyName="ItemNumber"
                                    ValidateRequiredField="true" OnSelectedIndexChanged="dropAddBudgetItemNumber_SelectedIndexChanged">
                                </ui:UIFieldDropDownList>
                                <ui:UIFieldDropDownList runat="server" ID="dropAddBudget" Caption="Budget" PropertyName="BudgetID" ValidateRequiredField="true"
                                ...
                                <ui:UIFieldDateTime runat="server" ID="dateAddBudgetStartDate" Caption="Start Date" PropertyName="StartDate"
                                    ValidateRequiredField="true" SelectMonthYear="true">
                                </ui:UIFieldDateTime>
                                <ui:UIFieldDateTime runat="server" ID="dateAddBudgetEndDate" Caption="End Date" ValidateRequiredField="true" PropertyName="EndDate"
                                    SelectMonthYear="true">
                                </ui:UIFieldDateTime>
                                <ui:UIFieldTreeList runat='server' ID="treeAddBudgetAccounts" Caption="Account" ValidateRequiredField="true" PropertyName="AccountID"
                                    OnAcquireTreePopulater="treeAddBudgetAccounts_AcquireTreePopulater">
                                </ui:UIFieldTreeList>
                                <ui:UIFieldTextBox runat='server' ID="textAddBudgetAmount" Caption="Amount" ValidateRequiredField="true" PropertyName="Amount"
