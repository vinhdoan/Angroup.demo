...
<script runat="server">
    ...
    protected void gridTasks_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        //7th Mar 2011, Joey: 
        //HIghlight request for quotation tasks with awarded ipt vendors
        if (e.Row.RowType == DataControlRowType.Footer && gridTasks.Rows.Count > 0)
        {
            List<String> rfqIDs = new List<String>();
            for (int i = 0; i < gridTasks.Rows.Count; i++)
            {
                if ((gridTasks.Rows[i].Cells[10].Text.CompareTo("ORequestForQuotation") == 0))
                    rfqIDs.Add(gridTasks.Rows[i].Cells[11].Text.Trim());
            }
            System.Data.DataTable dt = ORequestForQuotationItem.hasIptVendor(rfqIDs);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    ht.Add(dt.Rows[i][0].ToString(), dt.Rows[i][1]);
                }

                for (int i = 0; i < gridTasks.Rows.Count; i++)
                {
                    if (
                        (gridTasks.Rows[i].Cells[10].Text.CompareTo("ORequestForQuotation") == 0)
                        &&
                        ht.ContainsKey(gridTasks.Rows[i].Cells[11].Text)
                        )
                    {
                        for (int j = 0; j < gridTasks.Rows[i].Cells.Count; j++)
                            gridTasks.Rows[i].Cells[j].ForeColor = Color.Red;
                    }
                }
            }
            else
                    ...
                    <tr valign="top">
                        ...
                        <td>
                            ...
                            <ui:UIPanel runat="server" ID="panelTasks" BorderStyle="NotSet" meta:resourcekey="panelTasksResource1">
                                <ui:uiobjectpanel runat="server" id="panelMassAction" 
                                    ...
                                    <table cellpadding='0' cellspacing='0' border='0' >
                                        <tr>
                                            <td>
                                               ...
                                               &nbsp;
                                               &nbsp;
                                                <ui:uifieldtextbox runat="server" id="textComments" Caption="Comments" MaxLength="255"
