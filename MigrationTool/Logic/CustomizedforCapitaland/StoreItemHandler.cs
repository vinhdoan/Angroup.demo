...
{
    public class StoreItemHandlerForCapitaland : Migratable
    {
        public StoreItemHandlerForCapitaland(string mapfrom, string mapto)
            : base(mapfrom, mapto)
        { }

        public StoreItemHandlerForCapitaland(string mapfrom, string mapto, string sourcefile)
        ...
        private void ImportStoreBinItems(DataTable table)
        {
            ...
            foreach (DataRow dr in table.Rows)
            {
                try
                {
                    ...
                    //Fix store name as main store's name (RCS Store)
                    string storeName = ConvertToString(dr["Store"]);
                    if (storeName == null) continue;

                    string mapToColName = "Location";
                    string mapFromColName = map[mapToColName];
                    ...
                    string ctgName = stockcode;
                    string binDescription = ConvertToString(dr["Description"]);

                    mapToColName = "Avg-Cost";
                    mapFromColName = map[mapToColName];
                    ...
                    OStoreBinItem binItem = CreateStoreBinItem(storeName, binName, binDescription, ctgName, stockcode, unitPrice, qty);
        ...
        private OStoreBinItem CreateStoreBinItem(string storeName, string storeBinName, string storeBinDescription, string ctgName, string stockcode, decimal unitPrice, decimal qty)
        {
            //find store bin
            ...
            if (null == item)
            {
                OStore store = StoreHandler.CreateStore(storeName, null, new string[] { storeBinName }, new string[] { storeBinDescription });
